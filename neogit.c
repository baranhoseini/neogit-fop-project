...
void copydr(const char *srdir, const char *desdir)
{
    ...
    char destp[MAX_PATH];
    if (!CreateDirectory(desdir, NULL) && GetLastError() != ERROR_ALREADY_EXISTS)
    {
        printf("error occured; you are not in neogit repos or the directory has been added before\n");
        return;
    }
...
void compare(const char *dir1, const char *dir2)
{
    ...
    char pathh[MAX_PATH];
    char patht[MAX_PATH];
    HANDLE hFind = FindFirstFileA(strcat(strcpy(pathh, dir1), "\\*"), &findFileData);
    do
    {
        ...
        if (strcmp(filename, "useremail.txt") == 0)
            continue;
        if (strcmp(filename, "commited") == 0)
            continue;
        if (strcmp(filename, "staged") == 0)
            continue;
        if (strcmp(filename, "branch") == 0)
            continue;
        if (strcmp(filename, "commited") == 0)
            continue;
        if (strcmp(filename, "current_branch.txt") == 0)
            continue;
        ...
        {
            snprintf(pathh, sizeof(pathh), "%s\\%s", dir1, filename);
            snprintf(patht, sizeof(patht), "%s\\%s", dir2, filename);
            if (!CompareFileContents(pathh, patht))
            {
                printf("file %s = +M\n", filename);
            }
            else
...
void deletefiledr(const char *deletePath)
{
    char *cmd = malloc(strlen(deletePath) + strlen("del /s /q \"\"") + 1);
    if (cmd == NULL)
    {
        printf("Memory allocation failed.\n");
        return;
    }
    FILE *file = fopen(deletePath, "r");
    if (file != NULL)
    {
        fclose(file);
        sprintf(cmd, "del /q \"%s\"", deletePath);
    }
    else
    {
        sprintf(cmd, "rmdir /s /q \"%s\"", deletePath);
    }
...
void changebranch(const char *namebr)
{
    ...
    fclose(filebr);
    filebr = fopen("current_branch.txt", "w");
    fprintf(filebr, "%s", namebr);
    fclose(filebr);
    char currentDirectory[MAX_PATH];
    GetCurrentDirectory(MAX_PATH, currentDirectory);
    char destp[MAX_PATH];
    snprintf(destp, sizeof(destp), "%s\\branch\\%s", currentDirectory, crbrname);
    if (!CreateDirectory(destp, NULL) && GetLastError() != ERROR_ALREADY_EXISTS)
    {
        printf("error occurred; you are not in neogit repos or the directory has been added before\n");
        return;
    }
    char sourcep[MAX_PATH];
    snprintf(sourcep, sizeof(sourcep), "%s\\*", currentDirectory);
    WIN32_FIND_DATAA findFileData;
    HANDLE hFind;
    hFind = FindFirstFile(sourcep, &findFileData);
    if (hFind == INVALID_HANDLE_VALUE)
    ...
    do
    {
        if (strcmp(findFileData.cFileName, ".") != 0 && strcmp(findFileData.cFileName, "..") != 0)
        {
            snprintf(sourcep, sizeof(sourcep), "%s\\%s", currentDirectory, findFileData.cFileName);
            char destp[MAX_PATH];
            snprintf(destp, sizeof(destp), "%s\\branch\\%s\\%s", currentDirectory, crbrname, findFileData.cFileName);
            if (strcmp(findFileData.cFileName, "branch") == 0)
                continue;
            if (strcmp(findFileData.cFileName, "commited") == 0)
                continue;
            if (strcmp(findFileData.cFileName, "current_branch.txt") == 0)
                continue;
            if (strcmp(findFileData.cFileName, "username.txt") == 0)
                continue;
            if (strcmp(findFileData.cFileName, "useremail.txt") == 0)
                continue;
            if (strcmp(findFileData.cFileName, "staged") == 0)
            {
                copydr(sourcep, destp);
                continue;
            }
    ...
    char newbranchf[MAX_PATH];
    snprintf(newbranchf, sizeof(newbranchf), "%s\\branch\\%s", currentDirectory, namebr);
    CreateDirectory(newbranchf, NULL);

    char delstage[MAX_PATH];
    snprintf(delstage, sizeof(delstage), "%s\\staged\\*", currentDirectory);
    WIN32_FIND_DATAA finddFileData;
    HANDLE hFindd;
    hFindd = FindFirstFile(delstage, &finddFileData);
    ...
    do
    {
        if (strcmp(finddFileData.cFileName, ".") != 0 && strcmp(finddFileData.cFileName, "..") != 0)
        {
            snprintf(delstage, sizeof(delstage), "%s\\staged\\%s", currentDirectory, finddFileData.cFileName);

            deletefiledr(sourcep);
        }
    } while (FindNextFile(hFind, &finddFileData) != 0);
    FindClose(hFindd);
    char createstage[MAX_PATH];
    snprintf(createstage, sizeof(createstage), "%s\\staged", currentDirectory);
    CreateDirectory(createstage, NULL);
}
...
void checkoutbranch(const char *brachname)
{
    ...
    GetCurrentDirectory(MAX_PATH, currentDirectory);
    char txt[MAX_PATH];
    snprintf(txt, sizeof(txt), "%s\\commited\\commit_num.txt", currentDirectory);
    int num;
    FILE *comnum = fopen(txt, "r");
    fscanf(comnum, "%d", &num);
    fclose(comnum);
    for (int i = num; num > 0; i--)
    {
        char cmfillee[MAX_PATH];
        snprintf(cmfillee, sizeof(cmfillee), "%s\\commited\\branch_%d.txt", currentDirectory, i);
        ...
        if (strcmp(brachname, brrr) == 0)
        {
            ...
            snprintf(sourcee, sizeof(sourcee), "%s\\commited\\%d", currentDirectory, i);
...
void checkoutid(const char *id)
{
    ...
    char destinn[MAX_PATH];
    char sourcee[MAX_PATH];
    snprintf(sourcee, sizeof(sourcee), "%s\\commited\\%d", currentDirectory, newnum);
    snprintf(destinn, sizeof(destinn), "%s", currentDirectory);
    copydr(sourcee, destinn);
}
void checkouthead()
...
int main(int argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "reset") == 0)
    {
        if (argc == 3)
        {
            ...
            snprintf(delpath, sizeof(delpath), "%s\\staged\\%s", currentpath, last);
            deletefiledr(delpath);
        }
        else if (strcmp(argv[2], "-f") == 0)
        {
            for (int i = 3; i < argc; i++)
    ...
    else if (strcmp(argv[1], "status") == 0)
    {
        ...
        char destin[MAX_PATH];
        snprintf(destin, sizeof(destin), "%s\\staged", current);
        compare(current, destin);
    }
    else if (strcmp(argv[1], "commit") == 0)
    {
        ...
        else if (strcmp(argv[2], "-m") == 0)
        {
            if (argc == 4)
            {
                ...
                snprintf(txtfile, sizeof(txtfile), "%s\\commited\\commit_num.txt", currentDirectory);
                int num;
                FILE *fl = fopen(txtfile, "r");
                fscanf(fl, "%d", &num);
                fclose(fl);
                num = num + 1;
                char destin[MAX_PATH];
                snprintf(destin, sizeof(destin), "%s\\commited\\%d", currentDirectory, num);
                ...
                char branch[3000];
                FILE *fbr = fopen("current_branch.txt", "r");
                fscanf(fbr, "%s", branch);
                fclose(fbr);
                char brtxt[MAX_PATH];
                snprintf(brtxt, sizeof(brtxt), "%s\\commited\\branch_%d.txt", currentDirectory, num);
                FILE *bff = fopen(brtxt, "w");
                fprintf(bff, "%s", branch);
                fclose(bff);
                char commenttxt[MAX_PATH];
                snprintf(commenttxt, sizeof(commenttxt), "%s\\commited\\comment_%d.txt", currentDirectory, num);
                FILE *cf = fopen(commenttxt, "w");
                fprintf(cf, "%s", comment);
                fclose(cf);
                ...
                printf("commit made at: %s", asctime(localTime));
                char tmtxt[MAX_PATH];
                snprintf(tmtxt, sizeof(tmtxt), "%s\\commited\\time_%d.txt", currentDirectory, num);
    ...
    else if (strcmp(argv[1], "branch") == 0)
    {
        if (argc == 3)
        {
            ...
            if (strcmp(argv[2], brname) == 0)
                printf("this is your current branch");
            else
            {
                changebranch(argv[2]);
            }
